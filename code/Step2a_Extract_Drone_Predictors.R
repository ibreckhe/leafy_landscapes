...
## Set up workspace and load necessary libraries
library(rstanarm)
library(sf)
library(terra)
library(readr)
...
# Read plot polygons and extract visit code from plot labels
plot_polygons <- vect("./data/landscape_poly_nudge_wgs84.gpkg")

# Extract visit code from plot labels
plot_polygons$visit_code <- substr(plot_polygons$Label_of_F,1,3)

# Join polygons and image paths by visit code
plot_polygons_join <- merge(plot_polygons, image_paths, by = "visit_code", all.x = TRUE)
...
# Define functions to calculate green chromatic coordinate (GCC) and blue chromatic coordinate (BCC)
gcc_fun <- function(x){x[2] / (x[1] + x[2] + x[3])}
bcc_fun <- function(x){x[3] / (x[1] + x[2] + x[3])}

# Loop through each plot and extract data
for(i in 1:nrow(plot_polygons_join)){
  print(paste("Extracting data for plot", plot_polygons_join$Label_of_F[i]))

  # Crop and resample the field plots and store the output
  chip_list[[i]] <- crop_resample_field_plots(plot_vect = plot_polygons_join[i,],
                            ...
                            buffer_width = 0.96, write_output = FALSE)
  names(chip_list)[i] <- plot_polygons_join$Label_of_F[i]

  # Calculate summary statistics for each chip and store the results
...
# Calculate the square of the plot_gcc_mean and the leaf water percentage
